...
def reduce_mem_usage(df, verbose=True):
    ...
    if verbose and start_mem > 0:
        print(f"Memoria: {start_mem:.2f}MB -> {end_mem:.2f}MB ({100*(start_mem-end_mem)/start_mem:.1f}% menos)")
    return df
...
def preprocess_csv_to_hourly_features(csv_path, decimal=",", start_ts="2017-03-29 12:00:00", target_col=TARGET_COL):
    df = pd.read_csv(csv_path, decimal=decimal, parse_dates=["date"], index_col="date")
    df = df.loc[start_ts:]
    ...
    df_15 = df.resample("15min").first().drop("% Silica Concentrate", axis=1, errors="ignore")
...
def prepare_lstm_inputs(df_h, lstm_feats, scaler, target_col=TARGET_COL):
    ...
    df_lstm_scaled = pd.DataFrame(scaler.transform(df_lstm_only), index=df_lstm_only.index, columns=lstm_feats)
    if target_col in df_lstm_scaled.columns:
        X_lstm_infer = df_lstm_scaled.drop(columns=[target_col]).astype("float32")
    else:
...
def build_next_hour_features(last_row, last_ts, candidate_vec, reagent_cols, xgb_features, target_col=TARGET_COL):
    ...
    h = t_next.hour; d = t_next.dayofweek
    if "sin_hora" in row.index: row["sin_hora"] = np.sin(2*np.pi*h/24.0)
    if "cos_hora" in row.index: row["cos_hora"] = np.cos(2*np.pi*h/24.0)
    if "sin_dia_de_la_semana" in row.index: row["sin_dia_de_la_semana"] = np.sin(2*np.pi*d/7.0)
    if "cos_dia_de_la_semana" in row.index: row["cos_dia_de_la_semana"] = np.cos(2*np.pi*d/7.0)
...
def make_objective_xgb(df_h, xgb_model, xgb_features, reagent_cols, bounds_dict):
    ...
    def predict_with_xgb(x):
        ...
        X_next = build_next_hour_features(last_row, last_ts, np.array(xr, dtype=np.float32), reagent_cols, xgb_features)
...
def make_objective_lstm(df_h, lstm_model, lstm_feats, lstm_scaler, reagent_cols, bounds_dict, target_col=TARGET_COL):
    ...
    base_scaled = pd.Series(lstm_scaler.transform(base_row.to_frame().T)[0], index=lstm_feats)
    ...
    def predict_with_lstm(x):
        ...
        for i, c in enumerate(reagent_cols):
            if c in row_scaled.index:
                ...
                base_orig[c] = v
                row_scaled_local = pd.Series(lstm_scaler.transform(base_orig.to_frame().T)[0], index=lstm_feats)
                row_scaled[c] = row_scaled_local[c]
        # construir X (drop target si está)
        if target_col in row_scaled.index:
            X_vec = row_scaled.drop(labels=[target_col]).values.astype("float32").reshape(1, -1)
        else:
            X_vec = row_scaled.values.astype("float32").reshape(1, -1)
        pred_scaled = float(lstm_model.predict(X_vec, verbose=0).reshape(-1)[0])

        # desescalar predicción a unidades reales reconstruyendo vector completo
        recon = row_scaled.copy()
        recon[target_col] = pred_scaled
        recon_df = pd.DataFrame([recon.values], columns=lstm_feats, index=[last_idx + pd.Timedelta(hours=1)])
        pred_real = lstm_scaler.inverse_transform(recon_df.values)[0, lstm_feats.index(target_col)]
        return pred_real
...
def optimize_next_hour(df_h, reagent_cols, bounds_dict,
    ...
    if engine == "lstm":
        ...
        use_lstm = (lstm_model is not None and lstm_feats is not None and lstm_scaler is not None)

    if use_lstm:
        objective, ok = make_objective_lstm(df_h, lstm_model, lstm_feats, lstm_scaler, reagent_cols, bounds_dict)
        if not ok:
            # no hay palancas en features LSTM -> caer a XGB
            use_lstm = False

    if not use_lstm:
        if (xgb_model is None) or (xgb_features is None):
            raise ValueError("No hay XGBoost disponible para fallback.")
        objective = make_objective_xgb(df_h, xgb_model, xgb_features, reagent_cols, bounds_dict)
    ...
    last_row = df_h.iloc[-1]
    base_vec = np.array([last_row.get(c, np.nan) for c in reagent_cols], dtype=np.float32)

    # evaluar con XGB (para reporte)
    pred_base = pred_opt = np.nan
    if (xgb_model is not None) and (xgb_features is not None):
        last_ts = df_h.index[-1]
        X_base = build_next_hour_features(last_row, last_ts, base_vec, reagent_cols, xgb_features)
        X_opt  = build_next_hour_features(last_row, last_ts, bestx,   reagent_cols, xgb_features)
        pred_base = float(xgb_model.predict(xgb.DMatrix(X_base))[0])
        pred_opt  = float(xgb_model.predict(xgb.DMatrix(X_opt ))[0])

    return {
        "engine": "lstm" if use_lstm else "xgb",
...
def backtest_optimizer(df_h, xgb_model, xgb_features, reagent_cols, bounds_dict,
    ...
    for t0 in idx[:-1]:
        r0 = df_h.loc[t0]
        base_vec = np.array([r0.get(c, np.nan) for c in reagent_cols], dtype=np.float32)
        ...
        def obj_local(x):
            ...
            for i, c in enumerate(reagent_cols):
                lo, hi = bounds_dict[c]
                xr.append(float(np.clip(x[i], lo, hi)))
            Xn = build_next_hour_features(r0, t0, np.array(xr, dtype=np.float32), reagent_cols, xgb_features)
        ...
        Xb = build_next_hour_features(r0, t0, base_vec, reagent_cols, xgb_features)
        Xo = build_next_hour_features(r0, t0, bestx,   reagent_cols, xgb_features)
        ...
        rows.append({"t0": t0, "t1": t1, "baseline_pred": base_pred, "opt_pred": opt_pred, "real_next": real_next})
